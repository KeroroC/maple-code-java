...
- **锚点**：`TokenEstimator.estimate(List<ChatMessage>, TokenUsage? anchor)`：
  - 若 `anchor != null`，起点为 `anchor.inputTokens + anchor.cacheReadTokens + anchor.cacheCreationTokens`
  - 对 `messages` 的增量按 `chars / 4` 估
  - 增量 = 每个 block 的序列化文本长度：
    - `TextBlock.text` → `text.length()`
    - `ToolUseBlock` → JSON 序列化（id + name + input）的字符数
    - `ToolResultBlock` → `content.length()`（content 已经是 String）
  - 返回 `anchorTokens + delta`
- **首轮**（无锚点）：直接 `sum(delta) / 4`，结果会偏低 13K 余量覆盖
...
| 测试类 | 必须覆盖 |
|---|---|
| `TokenEstimatorTest` | (a) 空 list → 0；(b) 单 TextBlock 1000 char → 250 tokens ± 0；(c) 锚点 + delta：anchor=1000, delta=2000 char → 1500；(d) ToolResultBlock 100KB content → 25000 tokens（与 chars/4 一致） |
